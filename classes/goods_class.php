...
class goods_class
{
	...
	*/
    ...
    public static function getGoodsCategorySeller($goods_id){

        $gcQuery         = new IQuery('category_extend_seller as ce');
        $gcQuery->join   = "left join category_seller as c on c.id = ce.category_id";
        $gcQuery->where  = "ce.goods_id = '{$goods_id}'";
        $gcQuery->fields = 'c.name';

        $gcList = $gcQuery->find();
        $strCategorySellerNames = '';
        foreach($gcList as $val){
            $strCategorySellerNames .= $val['name'] . '    ';
        }
        unset($gcQuery,$gcList);
        return $strCategorySellerNames;
    }
     ...
     * @return mixed|string
     */
	public static function getGoodsTypes($goods_type){
        $result = array(
            1 => '点餐',
            2 => '购物',
        );
        return isset($result[$goods_type]) ? $result[$goods_type] : '';
    }
	/**
	 * @brief 返回检索条件相关信息
	...
	public static function getSearchCondition($search)
	{
        ...
        {
            $goods_type  = IFilter::act($search['goods_type'],'int');
            $where[] = "go.goods_type = ".$goods_type;
        }
