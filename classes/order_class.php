...
class Order_Class
{
	 ...
	 * @return string 订单状态说明  订单状态 1-未支付,2-提交中,3-支付成功,5完成订单,-1-取消订单(客户触发),-2作废订单(管理员触发)',
	 */
...
//			4 => '完成订单',
	...
	public static function getOrderGoods($order_id)
	{
		...
		$orderGoodsObj->fields = 'id,goods_name,goods_id,product_id,goods_nums';
		$orderGoodsList = $orderGoodsObj->find();
		$goodList = array();
		foreach($orderGoodsList as $good)
		{
//			$temp = JSON::decode($good['goods_array']);
            $temp['goods_name'] = $good['goods_name'];
