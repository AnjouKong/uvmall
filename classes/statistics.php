...
class statistics
{
	...
	public static function spandAmount($start = '',$end = '')
	{
         ...
         * */
        $db = new IQuery('order');
        $db->fields = 'sum(real_amount) as yValue,`create_time`';
        $db->where  = 'status >= 3';
        return self::ParseCondition($db,'create_time',$start,$end);
	}
	...
	public static function sellerAmount($seller_id,$start = '',$end = '')
	{
	    ...
	    */
        ...
        $db = new IQuery('order');
        $db->fields = 'sum(real_amount) as yValue,`create_time`';
        $db->where  = "seller_id = {$seller_id} and status >= 3";
        return self::ParseCondition($db,'create_time',$start,$end);
	...
	public static function sellCountSeller($seller_id)
	{
        ...
        $dataRow = $orderDB->getObj("seller_id = {$seller_id} and status >= 3",'count(id) as num');
        return $dataRow['num'];
	}
