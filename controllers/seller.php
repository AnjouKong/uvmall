...
class Seller extends IController implements sellerAuthorization
{
	...
	public function goods_update()
	{
		...
		unset($_POST['callback']);
         ...
         echo("<script>console.log('".json_encode($_POST['_imgList'])."');</script>");
	...
	public function goods_list()
	{
		...
		$goodHandle->order  = "go.sort desc,go.id desc";
		$goodHandle->fields = "distinct go.id,go.name,go.goods_type,go.sell_price,go.market_price,go.store_nums,go.img,go.is_del,go.seller_id,go.is_share,go.sort";
        ...
        // echo("<script>console.log('".json_encode($goodHandle)."');</script>");
	}
	...
	public function goods_report()
	{
		...
		$reportObj->setTitle(array("商品名称","类型","分类","售价","状态","库存"));
		foreach($goodList as $k => $val)
		{
			$insertData = array(
				$val['name'],
                goods_class::getGoodsTypes($val['goods_type']),
				goods_class::getGoodsCategorySeller($val['id']),
...
//		$where  = "o.seller_id = ".$seller_id." and o.if_del = 0 and o.status not in(3,4)";
        $where  = "o.seller_id = ".$seller_id." and o.if_del = 0";
		$where .= $condition ? " and ".$condition : "";
		$page   = IReq::get('page') ? IFilter::act(IReq::get('page'),'int') : 1;
	...
	public function order_report()
	{
        ...
        $reportObj->setTitle(array("订单编号","日期","房间号","订单金额","实际支付","订单状态","商品信息","订单备注"));
...
//				if ($good['value']!='') $strGoods .= " 规格：".$good['value'];
                $strGoods .= "商品名称：".$good['goods_name']."    数量：".$good['goods_nums'];
				$strGoods .= "<br />";
			}
...
//				Order_Class::getOrderPayStatusText($val),
				Order_Class::orderStatusText($val['status']),
				$strGoods,
				$val['note'],
			);
			$reportObj->setData($insertData);
	...
	function category_list()
	{
		...
		{
			$goods = new goods_class();
			$data = $goods->sortdata($tb_category->query('seller_id = '.$this->seller['seller_id'],'*','sort desc, id asc'));
