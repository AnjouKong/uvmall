...
class Member extends IController implements adminAuthorization
{
	...
	public function seller_edit()
	{
		...
		if($seller_id)
		{
			...
			$sellerRowTmp = $sellerDB->getObj('id = '.$seller_id);

            $seller_configDB = new IModel("seller_config");
            $sellerConfigRowTmp = $seller_configDB->getObj('id = '.$seller_id);

            // echo("<script>console.log('".json_encode($sellerRowTmp)."');</script>");
            // echo("<script>console.log('".json_encode($sellerConfigRowTmp['pay_type'])."');</script>");
            $sellerRowTmp['pay_type'] = $sellerConfigRowTmp['pay_type'];
            $sellerRowTmp['device_ip'] = $sellerConfigRowTmp['device_ip'];
            $sellerRowTmp['ali_app_id'] = $sellerConfigRowTmp['ali_app_id'];
            $sellerRowTmp['ali_private_key'] = $sellerConfigRowTmp['ali_private_key'];

            $this->sellerRow = $sellerRowTmp;
		}
	...
	public function seller_add()
	{
		...
		$truename    = IFilter::act(IReq::get('true_name'));
		// 新加
        $tenant_id    = IFilter::act(IReq::get('tenant_id'));
        $pay_type    = IFilter::act(IReq::get('pay_type'));
        $device_ip    = IFilter::act(IReq::get('device_ip'));
        $ali_app_id    = IFilter::act(IReq::get('ali_app_id'));
        $ali_private_key    = IFilter::act(IReq::get('ali_private_key'));
        ...
        else if($sellerDB->getObj("tenant_id = '{$tenant_id}' and id != {$seller_id}"))
        {
            $errorMsg = "商户ID重复";
        }
		...
		if(!$seller_id)
		{
            ...
            $seller_config_id = $sellerDB->getObj("tenant_id = '{$tenant_id}'");
            // echo("<script>console.log('".json_encode($seller_config_id['id'])."');</script>");
            //待更新的数据
            $seller_configRow = array(
                'id'                  => $seller_config_id['id'],
                'tenant_id'           => $tenant_id,
                'pay_type'           => $pay_type,
                'device_ip'           => $device_ip,
            ...
            $seller_configDB->setData($seller_configRow);
            $seller_configDB->add();
		}
		//编辑会员
		else
		{
            ...
            $seller_configDB->setData($seller_configRow);
            $seller_configDB->update("id = ".$seller_id);
		}
		$this->redirect('seller_list');
	}
	/**
