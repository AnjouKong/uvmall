...
class Goods extends IController implements adminAuthorization
{
	...
	public function goods_img_upload()
	{
        ...
        /**
        $photoObj = new PhotoUpload();
        $result   = current($photoObj->runOSS());

        $uploadAPIUrl = IWeb::$app->config['uploadAPIUrl'];
        $ch = curl_init();
        curl_setopt($ch , CURLOPT_URL , $uploadAPIUrl);
        curl_setopt($ch , CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch , CURLOPT_POST, 1);
        curl_setopt($ch , CURLOPT_POSTFIELDS, $_FILES);
        $resultOSS = curl_exec($ch);
        curl_close($ch);


        // $result   =  sendFile_post($uploadAPIUrl, $_FILES);
        echo JSON::encode($result);
         * */
	}
     ...
     * @return bool|string
     */
    public function sendFile_post($url,$post_datas) {
...
//        $postdata = http_build_query($post_datas);
//        $options = array(
//            'http' => array(
//                'method' => 'POST',
...
//        return $result;
	...
	function goods_list()
	{
		...
		$goodsHandle->fields = "distinct go.id,go.name,go.goods_type,go.sell_price,go.market_price,go.store_nums,go.img,go.is_del,go.seller_id,go.is_share,go.sort";
